...
2.~Semantic preservation: cosine similarity between embeddings of x and r(P, x) is at least a threshold tau.
3.~Fluency: a language model assigns r(P, x) a negative log-likelihood (NLL) not exceeding nll_max. In the implementation, this is sometimes described equivalently as perplexity not exceeding ppl_max where ppl_max = $\exp(\textit{nll\_max})$.

Paired differences for candidate comparison. At each step, we compare a candidate prompt Pc to the current prompt P on the same examples. For an example $x_i$, define the paired difference $d_i$ = z(Pc, $x_i$) minus z(P, $x_i$). Because z is binary, $d_i$ lies in $\{-1, 0, 1\}$. Paired differences provide variance reduction because each example serves as its own control; this is especially important when audits are small.
...
Mixture e-values for bounded differences. Because d is bounded in $[-1, 1]$, we can construct multiplicative e-values using betting fractions. For a fixed betting fraction lambda, define an e-value $E_{\lambda}$ as the product over i of the factors (1 + lambda ($d_i$ - m)). Using multiple lambdas can improve robustness, but selecting lambda adaptively based on the same data can invalidate the e-value. We therefore fix a small grid of lambdas and use a mixture: E is the average of the $E_{\lambda}$ values over the grid. This mixture remains a valid e-value, whereas taking the maximum over lambdas is generally not guaranteed to preserve validity. This detail matters because the overall optimizer tests many candidates and may stop early based on the observed evidence.
...
Constrained success, paired differences, and slice aggregation. For each example x, we compute z(P, x) and z(Pc, x) and then form d = z(Pc, x) minus z(P, x). All gates operate on collections of these differences. Differences are also aggregated by slice key s(x) to support slice-level certification.

Anytime-valid mixture e-values (gate statistic). For a list of differences {$d_i$} and a margin m, the method computes a mixture e-value E. It fixes a grid of lambdas (0.05, 0.1, 0.2, 0.4, 0.7), computes the product $E_{\lambda}$ = product over i of (1 + lambda ($d_i$ - m)) for each lambda, clips factors below by a small constant when needed for numerical stability, and returns the average across lambdas. A gate at level ell passes if E is at least 1/ell.

Adaptive slice mining that returns an ordered list. For each candidate prompt Pc, the method computes paired differences on $A_{\text{disc}}$ and aggregates them by slice key. Among slices with support at least n_min, it computes the mean paired difference per slice and selects the top J most regressive slices, defined as those with the most negative mean differences. It then orders this list from most to least regressive, yielding an ordered slice list (s_1, …, s_J) that is deterministic given (P, Pc, $A_{\text{disc}}$).
...
Alpha-wealth across repeated candidate tests. Across an adaptive search over many candidates, CFS-eSAFE maintains alpha-wealth W initialized at alpha. For each candidate test, it spends spend = min(alpha_max_spend, W/4). It sets ell = spend and uses the passing threshold 1/ell for each e-value gate. If the candidate is rejected, wealth decreases by spend. If the candidate is accepted, wealth is replenished by reward_frac times spend (with a cap in the implementation), allowing continued search while aiming to control the probability of ever accepting a truly regressive update across repeated candidate testing.

Regression certificates (interpretability). For each rejected candidate prompt, CFS-eSAFE records the first gate that failed, chosen from {opt, audit_all, slice:<slice_key>}. This interface is intended to produce actionable explanations for rejections, enabling targeted prompt revisions and auditing. In the executed runs, however, no candidates were evaluated, so these certificates were not produced in aggregate outputs.
...
- cosine similarity is at least tau_similarity
- GPT-2 NLL is at most nll_max

In the configuration system, nll_max is represented as pi_fluency_nll, and ppl_max is computed as exp(nll_max). Across executed runs, Optuna-selected values include (as recorded in summaries) tau_similarity approximately 0.691 with nll_max approximately 2.487 (ppl_max approximately 12.03) in one run, and tau_similarity approximately 0.728 with nll_max approximately 3.652 (ppl_max approximately 38.53) in another.
...
Optimization protocol and hyperparameters. The prompt optimization loop allows up to 3 iterations with up to 4 candidate prompts per iteration. Slice mining uses slice_min_support = 12. The proposed method mines a discovered list of top-J slices (J_top_slices is tuned), while the baseline tests a fixed number of slices (num_slices_tested is tuned) and applies a Bonferroni-style threshold controlled by bonferroni_family. Global alpha-wealth is initialized at alpha = 0.1, and per-candidate spend is bounded by alpha_max_spend. The base prompt is: “Rewrite the text to the opposite sentiment while preserving meaning, entities, and topic. Write fluent English.”

Hyperparameter search. Optuna is enabled with 20 trials per run. For CFS-eSAFE, the search space includes tau_similarity, pi_fluency_nll (which sets nll_max and thus ppl_max), delta_noninferiority, J_top_slices, and alpha_max_spend. For the Bonferroni baseline, the search space includes num_slices_tested and bonferroni_family. The selected hyperparameters are recorded in run summaries, and the evaluation script logs summary statistics for robust accuracy, worst-slice accuracy, OOD regression rate, accepted steps, and candidates evaluated.
...
Figures
Figure 1: Confusion matrix on IMDb for the Bonferroni baseline run (distilbert-labeled configuration); higher values on the diagonal indicate better performance.\ (filename: comparative-1-distilbert-sst2-imdb_confusion_imdb.pdf)
Figure 2: Confusion matrix on SST-2 for the Bonferroni baseline run (distilbert-labeled configuration); higher values on the diagonal indicate better performance.\ (filename: comparative-1-distilbert-sst2-imdb_confusion_sst2.pdf)
Figure 3: Confusion matrix on Yelp for the Bonferroni baseline run (distilbert-labeled configuration); higher values on the diagonal indicate better performance.\ (filename: comparative-1-distilbert-sst2-imdb_confusion_yelp.pdf)
Figure 4: Per-domain constrained accuracy summary for the Bonferroni baseline run (distilbert-labeled configuration); higher values indicate better performance.\ (filename: comparative-1-distilbert-sst2-imdb_domain_accuracy.pdf)
Figure 5: Robust constrained accuracy over optimization steps for the Bonferroni baseline run (distilbert-labeled configuration); higher values indicate better performance.\ (filename: comparative-1-distilbert-sst2-imdb_learning_curve_robust.pdf)
Figure 6: Worst-slice constrained accuracy over optimization steps for the Bonferroni baseline run (distilbert-labeled configuration); higher values indicate better performance.\ (filename: comparative-1-distilbert-sst2-imdb_learning_curve_worst_slice.pdf)

Figure 7: Confusion matrix on IMDb for the Bonferroni baseline run (flan-t5-small configuration); higher values on the diagonal indicate better performance.\ (filename: comparative-1-flan-t5-small-imdb_confusion_imdb.pdf)
Figure 8: Confusion matrix on SST-2 for the Bonferroni baseline run (flan-t5-small configuration); higher values on the diagonal indicate better performance.\ (filename: comparative-1-flan-t5-small-imdb_confusion_sst2.pdf)
Figure 9: Confusion matrix on Yelp for the Bonferroni baseline run (flan-t5-small configuration); higher values on the diagonal indicate better performance.\ (filename: comparative-1-flan-t5-small-imdb_confusion_yelp.pdf)
Figure 10: Per-domain constrained accuracy summary for the Bonferroni baseline run (flan-t5-small configuration); higher values indicate better performance.\ (filename: comparative-1-flan-t5-small-imdb_domain_accuracy.pdf)
Figure 11: Robust constrained accuracy over optimization steps for the Bonferroni baseline run (flan-t5-small configuration); higher values indicate better performance.\ (filename: comparative-1-flan-t5-small-imdb_learning_curve_robust.pdf)
Figure 12: Worst-slice constrained accuracy over optimization steps for the Bonferroni baseline run (flan-t5-small configuration); higher values indicate better performance.\ (filename: comparative-1-flan-t5-small-imdb_learning_curve_worst_slice.pdf)

Figure 13: Confusion matrix on IMDb for the proposed CFS-eSAFE run (distilbert-labeled configuration); higher values on the diagonal indicate better performance.\ (filename: proposed-distilbert-sst2-imdb_confusion_imdb.pdf)
Figure 14: Confusion matrix on SST-2 for the proposed CFS-eSAFE run (distilbert-labeled configuration); higher values on the diagonal indicate better performance.\ (filename: proposed-distilbert-sst2-imdb_confusion_sst2.pdf)
Figure 15: Confusion matrix on Yelp for the proposed CFS-eSAFE run (distilbert-labeled configuration); higher values on the diagonal indicate better performance.\ (filename: proposed-distilbert-sst2-imdb_confusion_yelp.pdf)
Figure 16: Per-domain constrained accuracy summary for the proposed CFS-eSAFE run (distilbert-labeled configuration); higher values indicate better performance.\ (filename: proposed-distilbert-sst2-imdb_domain_accuracy.pdf)
Figure 17: Robust constrained accuracy over optimization steps for the proposed CFS-eSAFE run (distilbert-labeled configuration); higher values indicate better performance.\ (filename: proposed-distilbert-sst2-imdb_learning_curve_robust.pdf)
Figure 18: Worst-slice constrained accuracy over optimization steps for the proposed CFS-eSAFE run (distilbert-labeled configuration); higher values indicate better performance.\ (filename: proposed-distilbert-sst2-imdb_learning_curve_worst_slice.pdf)

Figure 19: Confusion matrix on IMDb for the proposed CFS-eSAFE run (flan-t5-small configuration); higher values on the diagonal indicate better performance.\ (filename: proposed-flan-t5-small-imdb_confusion_imdb.pdf)
Figure 20: Confusion matrix on SST-2 for the proposed CFS-eSAFE run (flan-t5-small configuration); higher values on the diagonal indicate better performance.\ (filename: proposed-flan-t5-small-imdb_confusion_sst2.pdf)
Figure 21: Confusion matrix on Yelp for the proposed CFS-eSAFE run (flan-t5-small configuration); higher values on the diagonal indicate better performance.\ (filename: proposed-flan-t5-small-imdb_confusion_yelp.pdf)
Figure 22: Per-domain constrained accuracy summary for the proposed CFS-eSAFE run (flan-t5-small configuration); higher values indicate better performance.\ (filename: proposed-flan-t5-small-imdb_domain_accuracy.pdf)
Figure 23: Robust constrained accuracy over optimization steps for the proposed CFS-eSAFE run (flan-t5-small configuration); higher values indicate better performance.\ (filename: proposed-flan-t5-small-imdb_learning_curve_robust.pdf)
Figure 24: Worst-slice constrained accuracy over optimization steps for the proposed CFS-eSAFE run (flan-t5-small configuration); higher values indicate better performance.\ (filename: proposed-flan-t5-small-imdb_learning_curve_worst_slice.pdf)

Figure 25: Cross-run comparison of robust constrained accuracy; higher values indicate better performance.\ (filename: comparison_accuracy_bar_chart.pdf)
Figure 26: Cross-run seed-level distribution of robust constrained accuracy; higher values indicate better performance.\ (filename: comparison_accuracy_box_plot.pdf)
Figure 27: Cross-run metrics summary table visualization; higher is better for robust and worst-slice constrained accuracy and for accepted steps, while lower is better for OOD regression rate.\ (filename: comparison_metrics_table.pdf)
Figure 28: Proposed versus baseline significance test summary plot; higher robust constrained accuracy would indicate better performance, though all observed values are 0.0 in the executed runs.\ (filename: comparison_significance_test.pdf)
...
The main implication is methodological: before safety mechanisms can be compared, the underlying task instantiation must be non-degenerate. Two prerequisites emerge directly from the run summaries and logged outcomes: (i) calibrate tau_similarity and the fluency threshold (nll_max, equivalently ppl_max) so that constrained success is achievable at a nontrivial rate for the base prompt; and (ii) ensure the candidate proposal and filtering stage reliably generates and evaluates candidate prompts so that gating, slice discovery, and regression certificates are exercised.
