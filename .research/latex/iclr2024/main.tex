...
3) Fluency: a language model assigns r(P, x) a negative log-likelihood (NLL) not exceeding nll_max. In the implementation, this is sometimes described equivalently as perplexity not exceeding ppl_max where ppl_max = exp(nll_max).
...
- an identity-term flag indicating whether the text contains one of the terms {muslim, black, gay, disabled}
...
- D_opt: an optimization batch used to identify failures and to guide candidate prompt proposals. D_opt provides no safety guarantee.
- A: an audit set that is never used to propose edits. A is split into A_disc for slice discovery and ordering and A_conf for statistical confirmation.

This split is fixed once per seed. For each candidate prompt Pc, slices and their order are determined using only A_disc, and the resulting ordered slice list is treated as fixed when testing on A_conf. This cross-fitting aims to prevent slice-selection leakage: the evidence used to choose which slices to test is not reused to pass those slice tests.
...
1) Optimization improvement gate on D_opt: test H0_opt: expected d is at most 0.
2) Overall audit non-regression gate on A_conf: test H0_all: expected d is at most −delta.
3) Slice-list non-regression gates on A_conf restricted to each discovered slice s_j, in the A_disc order: test H0_sj: expected d restricted to slice s_j is at most −delta.
...
Datasets and splits. The optimization dataset D_opt is a subset of SST-2 train with 120 examples. The audit pool (never used to propose edits) is formed from:
...
From this pool, we sample an audit set A of size 200 and split it once into A_disc (100) and A_conf (100).
